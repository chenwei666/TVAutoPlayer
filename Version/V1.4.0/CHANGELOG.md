...
- 开发人员：chenwei666
- 版本类型：中英文界面与开源项目国际化功能版本

### 新增功能

- 新增完整英文 Android 字符串与复数资源，覆盖播放页、设置页、内置媒体库、权限申请、错误提示和确认对话框。
- 新增电视系统语言自适应：英文系统显示英文，中文系统显示中文，其他语言回退到默认中文。
- README 首页新增中英文项目简介、语言导航和开发人员信息。
- 新增 `README_EN.md` 完整英文说明，覆盖功能、安装、语言行为、开机启动、格式、构建、隐私和许可证。
- 新增 `scripts/validate-localization.ps1` 自动化语言资源质量门禁。
- 新增 `docs/releases/v1.4.0.md` 和 `Version/V1.4.0` 完整项目快照。

### 问题修复

- 修复英文系统电视仍只能显示中文界面的问题。
- 修复 GitHub About 只介绍视频播放、没有体现图片轮播和英文受众的问题。
- 修复后续新增中文 UI 文案可能遗漏英文翻译或格式参数不一致而无法在构建前发现的问题。

### 优化内容

- 默认中文资源标注 `tools:locale="zh"`，便于 Android Lint 正确识别源语言。
- 构建脚本在 Gradle 任务前检查中英文字符串键、复数数量、格式参数、空翻译和中文硬编码。
- GitHub Actions、本地构建脚本、README 和部署文档统一使用 V1.4.0 产物名称。
- 播放器业务逻辑、媒体格式、播放顺序、图片时长和失败跳过策略保持不变。

### 删除内容

- 无功能删除。
- 未删除中文界面、图片/视频循环、内置媒体库、系统文件选择器、开机播放或历史配置兼容逻辑。

### 修改文件

- 新增 `app/src/main/res/values-en/strings.xml`、`README_EN.md`、`scripts/validate-localization.ps1`。
- 修改默认中文 `strings.xml` 的源语言标记。
- 修改 `README.md`、`app/build.gradle.kts`、`scripts/build.ps1`、`.github/workflows/android.yml`。
- 修改架构、测试和部署文档，新增 `docs/releases/v1.4.0.md`。
- 新增 `Version/V1.4.0` 完整项目快照和 V1.4.0 APK 校验文件。

### 影响模块

- Android 资源选择、全部用户可见文案、README、GitHub About、构建质量门禁、持续集成和发布流程。
- 播放器、媒体库、权限判断、播放列表持久化和开机广播业务逻辑不受影响。
...
- 无网络 API 或外部业务接口变更。
- Android 资源新增英文限定目录 `values-en`；原资源 ID 保持不变。

### 配置变更

- `versionCode` 从 7 升至 8，`versionName` 从 1.3.1 升至 1.4.0。
- 构建产物名称升级为 `TVAutoPlayer-v1.4.0-debug.apk`。
- 构建质量门禁新增语言资源验证步骤。

### 兼容性说明

- 最低 Android 6.0（API 23）和目标 Android 16（API 36）不变。
- V1.0.0～V1.3.1 的播放列表、URI 权限、图片时长、声音、画面和开机设置可直接保留。
- 应用 ID、资源 ID 和 Debug 签名保持不变，可覆盖安装。
- 电视切换系统语言后，Android 重建或重新启动应用时加载对应语言资源。

### 升级方式

- 相同签名可执行 `adb install -r TVAutoPlayer-v1.4.0-debug.apk` 覆盖安装并保留设置。
- 不同签名无法直接覆盖；卸载重装会清除应用配置，但不会删除原媒体文件。

### 已知问题

- 发布 APK 使用 Android Debug 证书；正式批量部署应使用企业固定 Release 证书。
- 当前没有连接真实 Android TV 或 TV 模拟器，中英文长文本在具体电视字体、过扫描和焦点环境下仍需目标硬件验收。
- Android 10+ 及部分电视厂商可能限制普通第三方应用从开机广播直接拉起界面。

### 备注

- 本版本质量门禁包括语言资源验证、单元测试、Android Lint 和 Debug APK 构建。
- 发布 APK SHA-256：`03f687de57fe6c84467d2714e8406d63f8b0067aa16351f361b076551d9cc4b7`。
