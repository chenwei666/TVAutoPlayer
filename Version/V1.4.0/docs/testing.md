...
`scripts/validate-localization.ps1` 会在每次正式构建前检查：

- 默认中文与英文资源的字符串键和复数键完全一致。
- `%1$s`、`%1$d` 等格式参数在两种语言中完全一致。
- 英文翻译没有空值。
- Java、Manifest、布局和普通 XML 中没有绕过字符串资源的中文硬编码 UI 文案。
...
V1.4.0 本地质量门禁已执行语言资源验证、`testDebugUnitTest`、`lintDebug` 和 `assembleDebug`；Lint 已检查英文资源缺失和格式占位符一致性，APK 已通过包清单、语言资源、权限、ZIP 对齐、v1/v2 调试签名和 SHA-256 核验。验证开发机没有连接 Android TV、电视盒子或 TV 模拟器，因此无法执行中文/英文电视 UI 的真实布局、图片渲染、时间精度、媒体索引、U 盘、遥控器和厂商开机策略测试。自动化测试不替代目标硬件验收。
