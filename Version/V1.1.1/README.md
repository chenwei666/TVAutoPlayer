...
面向 Android TV / 电视盒子的本地视频循环播放应用。首次启动可一次选择多个视频，应用会保存这些文件的长期只读授权；以后打开应用会直接按选择顺序全屏循环播放，也可在电视开机后自动尝试启动。

## 已实现功能

- 首次启动自动进入系统文件选择器，可一次选择 1 个或多个视频。
...
[下载 TVAutoPlayer v1.1.1](https://github.com/chenwei666/TVAutoPlayer/releases/tag/v1.1.1)
...
```powershell
adb install -r .\TVAutoPlayer-v1.1.1-debug.apk
```

首次运行：

1. 打开“电视自动播放器”。
2. 在系统文件选择器中长按或勾选 U 盘/电视存储里的 1 个或多个视频。
3. 选择完成后立即按文件选择器返回的顺序循环播放。
4. 播放中按遥控器“返回”键进入设置，可重新选择整个列表或继续追加视频。
...
应用只申请 `RECEIVE_BOOT_COMPLETED`。视频通过 Android Storage Access Framework 由用户明确选择，应用只保存播放列表中各文件的 URI、显示名称和顺序，不扫描存储、不复制视频、不访问网络、不连接数据库。替换或清空列表会释放不再使用的文件授权；卸载应用会清除设置和授权，但不会删除原视频。
