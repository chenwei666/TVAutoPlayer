...
- `PlaybackConfiguration`：不可变配置值对象。
- `PlaybackPolicy`：决定当前应播放、选片还是提示授权失效；同时决定开机时是否应启动。
- `PlaybackConfigStore`：使用私有 SharedPreferences 保存少量非敏感设置。
- `UriAccess`：只对用户明确选择的 URI 检查可读性并获取显示名称。
- `MainActivity`：创建/释放 ExoPlayer、循环播放、全屏与异常恢复。
- `SettingsActivity`：系统文件选择、更换/清除文件、声音和画面设置。
- `BootCompletedReceiver`：在满足“已选视频、授权有效、开机启动开启”时尝试拉起主界面。

## 数据流

```text
系统文件选择器 -> 持久 URI 授权 -> PlaybackConfigStore
                                     |
...
- `REPEAT_MODE_ONE` 负责无缝循环。
- 授权失效时不创建播放器，转入重新选片流程。
