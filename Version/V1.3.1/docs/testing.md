...
V1.3.1 本地质量门禁已执行 `testDebugUnitTest`、`lintDebug` 和 `assembleDebug`；APK 已通过包清单、权限、ZIP 对齐、v1/v2 调试签名和 SHA-256 核验。验证开发机没有连接 Android TV、电视盒子或 TV 模拟器，因此无法执行真实图片渲染、时间精度、媒体索引、U 盘、遥控器和厂商开机策略测试。自动化测试不替代目标硬件验收。
