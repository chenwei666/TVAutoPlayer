...
public final class MainActivity extends ComponentActivity {
    ...
    private void startConfiguredPlayback() {
        ...
        if (!unreadableNoticeShown && readableItems.size() < configuration.videoCount()) {
            unreadableNoticeShown = true;
            Toast.makeText(
                    this,
                    getResources().getQuantityString(
                            R.plurals.unreadable_videos_skipped,
                            configuration.videoCount() - readableItems.size(),
                            configuration.videoCount() - readableItems.size()
                    ),
                    Toast.LENGTH_LONG
            ).show();
        ...
        for (PlaylistItem item : readableItems) {
            mediaItems.add(MediaItem.fromUri(Uri.parse(item.uri())));
        }
